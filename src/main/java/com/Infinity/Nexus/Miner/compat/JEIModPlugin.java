...
import com.Infinity.Nexus.Miner.compat.jei.MinerJeiCategory;
...
public class JEIModPlugin implements IModPlugin {
    ...
    public void registerCategories(IRecipeCategoryRegistration registration) {
        //-----------------------------------------Registry--------------------------------------------------//
        registration.addRecipeCategories(new MinerJeiCategory(registration.getJeiHelpers().getGuiHelper()));
    ...
    public void registerRecipes(IRecipeRegistration registration) {
        ...
        //-----------------------------------------Registry--------------------------------------------------//
        List<MinerRecipe> minerRecipes = recipeManager.recipeMap().byType(ModRecipes.MINER_RECIPE_TYPE.get()).stream().map(RecipeHolder::value).toList();

        try {
            registration.addRecipes(MinerJeiCategory.MINER_TYPE, minerRecipes);
    ...
    public void registerRecipeCatalysts(IRecipeCatalystRegistration registration) {
        registration.addCraftingStation(MinerJeiCategory.MINER_TYPE, ModBlocksMiner.MINER);
    }
    @Override
    public void registerGuiHandlers(IGuiHandlerRegistration registration) {
        //-----------------------------------------Registry--------------------------------------------------//
        registration.addRecipeClickArea(MinerScreen.class,162, -10,8,9, MinerJeiCategory.MINER_TYPE);
    }
